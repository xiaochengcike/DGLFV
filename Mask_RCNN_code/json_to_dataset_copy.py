...
def batch_solve(json_file,out_dir):
...
if __name__=="__main__":
    ...
    json_path = 'train_finger/json'
    out_path = 'train_finger/labelme_json'
    
    json_file = os.listdir(json_path)
    
    for jf in json_file:
        filename = jf.split('.')[0]+'_json'
        if not os.path.exists(os.path.join(out_path,filename)):
            os.mkdir(os.path.join(out_path,filename))
        batch_solve(os.path.join(json_path,jf),os.path.join(out_path,filename))
