...
def getRoiHCut2(img, p0):
    ...
    maxTop = np.argmax(img[0:int(h / 2) , 0])
    minTop = np.argmax(img[0:int( h / 2), w-1])

    maxBottom = np.argmax(img[int((3 * h / 4)): h, 0]) + 3 * h / 4
    minBottom = np.argmax(img[int((3 * h / 4)): h, w-1]) + 3 * h / 4

    maxTop = (2*maxTop + minTop) / 3
    maxBottom = (maxBottom + 2*minBottom) / 3

    return img[int(maxTop):int(maxBottom),:]
...
def getRoi(img):
    ...
    w = img.copy()
    w1 = w[int(heightDist):int(3 * heightDist),int(width / 4):]
    p0 = brightestColumn(w1) + heightDist + height / 2
    pCol = w[:,int(p0):int(p0 + 1)]

    pColInv = pCol[::-1]

    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))

    w1_2 = clahe.apply(w[:, int((p0 / 20)):int((p0 + p0 / 2))])
...
imgs = []
dbDir = os.getcwd() + "/Finger Vein Database/"
dbsave = os.getcwd() + "/gab_roi_db/"
...
for index, imgPath  in enumerate(imgs):
    ...
    cv2.imwrite(os.path.join(dbsave,p0Imgs[index].split('Database\\')[1] + ".png"), res0)
    print (index)
